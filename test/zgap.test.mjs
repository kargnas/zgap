...
`);
  ...
  assert.equal(userSkipResult.code, 5, userSkipResult.stderr);
  const userSkipInvocation = JSON.parse(userSkipResult.stdout);
  assert.equal(userSkipInvocation.env.OMP_SKIP_SETUP, "0");
...
test("OMP extension은 세션마다 다시 로드돼도 provider를 매번 등록한다", async (t) => {
  const root = await tempDir(t);
  const configRoot = path.join(root, "config");
  const configDir = path.join(configRoot, "zgap");
  await mkdir(configDir, { recursive: true });
  ...
  t.after(() => gateway.close());
  const fetchRedirectModule = await installGatewayFetchRedirect(t, gateway.address().port);
  await writeFile(path.join(configDir, "credentials.json"), JSON.stringify({ api_key: "zgap-file-key" }), { mode: 0o600 });
  const script = path.join(root, "double-load.mjs");
  // OMP rebuilds every session's extensions by importing this module again under a fresh
  // cache-busting specifier: the parent session, then one more time per subagent. A load that
  // consumes a one-shot resource would leave every subagent without the zzgg provider.
  await writeFile(script, `const registrations = [];
const pi = {
  registerFlag() {},
  registerProvider(name, config) { registrations.push({ name, apiKey: config.apiKey }); },
  async setModel() { return true; },
  on() {},
};
for (const tag of ["1", "2"]) {
  const module = await import(${JSON.stringify(path.join(repoDir, "src", "omp-provider-extension.mjs"))} + "?mtime=" + tag);
  await module.default(pi);
}
process.stdout.write(JSON.stringify(registrations));
`);
  // runOmp passes the handshake flag on the OMP command line; the extension reads it from argv.
  const child = spawn(nodePath, [script, `--zgap-provider-override-required-${"a".repeat(32)}=true`], {
    env: {
      HOME: path.join(root, "home"),
      PATH: process.env.PATH,
      XDG_CONFIG_HOME: configRoot,
      ZGAP_RUNTIME: process.execPath,
      NODE_OPTIONS: `--import=${fetchRedirectModule}`,
    },
  ...
  // Two loads, each registering the model overlay and the transport config.
  assert.deepEqual(registrations.map(({ name }) => name), ["zzgg", "zzgg", "zzgg", "zzgg"]);
  const keys = new Set(registrations.map(({ apiKey }) => apiKey));
  assert.equal(keys.size, 1);
  assert.match([...keys][0], /^!.*auth-token/);
});
